...
pub fn get_crc32(buf: &[u8]) -> u32 {
    ...
    while buf.len() >= 16 {
        result = CRC32_TABLE[0x0][buf[0xf] as usize]
            ...
            ^ CRC32_TABLE[0xc][buf[0x3] as usize ^ ((result >> 0x18) & 0xFF) as usize]
            ^ CRC32_TABLE[0xd][buf[0x2] as usize ^ ((result >> 0x10) & 0xFF) as usize]
            ^ CRC32_TABLE[0xe][buf[0x1] as usize ^ ((result >> 0x08) & 0xFF) as usize]
            ^ CRC32_TABLE[0xf][buf[0x0] as usize ^ (result & 0xFF) as usize];
        buf = &buf[16..];
    }
    update_slow(!result, buf)
}
